...
Util.events(board, {
	...
	"move": function(e) {
		...
		var candy = board.getCandyAt(row, col);
		var color = candy.color;
		var cell = dom.gameBoard.rows[row+1].cells[col+1].querySelector('div');
		var img = document.createElement("img");
		img.setAttribute('src', 'graphics/'+color+'-candy.png');
		...
		img.style.setProperty('--move-amount', Math.abs(move[1]));
		img.style.setProperty('--duration-move', .1*Math.abs(move[1]) + 's');
...
function getMoveDetails(e){
	...
	if (fromRow === undefined || fromCol === undefined){
		...
		return [DIR.DOWN, diffRow];
	}
	else if (diffRow < 0){//
		return [DIR.UP, diffRow];
	}
	else if (diffCol > 0){//right
		return [DIR.RIGHT, diffCol];
	}
	else if (diffCol < 0){//left
		return [DIR.LEFT, diffCol];
	}
}
